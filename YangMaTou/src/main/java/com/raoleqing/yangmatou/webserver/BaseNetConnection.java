            ...
            new AsyncTask<Void, Void, ResultData>() {
                ...
                protected ResultData doInBackground(Void... params) {
                    try {
                        ...
                        if (status.equals(Constant.NET_STATUS_SUCCESS)) {
                            if (json.has(Constant.DATA))
                                return new ResultData(BaseNetConnection.Status.Success, json.getJSONObject(Constant.DATA));
                            else
                                return new ResultData(BaseNetConnection.Status.Success, new JSONObject());
                        } else {
                ...
                protected void onPostExecute(ResultData resultData) {
                    try {
                        if (resultData.getStatus() == BaseNetConnection.Status.Success) {
                            ...
                            ToastUtil.MakeShortToast(YangMaTouApplication.getAppContext(),resultData.getData().optString(Constant.INFO));
