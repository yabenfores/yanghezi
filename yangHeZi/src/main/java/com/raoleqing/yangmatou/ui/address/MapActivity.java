...
import com.raoleqing.yangmatou.ben.BaiduMapHelper;
...
//        lvLocNear.setAdapter(adapter);
        ...
        lvLocNear.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
                ...
                inent.putExtra("province", province);
                inent.putExtra("city", city);
                inent.putExtra("district", district);
...
//                }).start();
    ...
    private void initMap() {
        // 定位初始化
        mBaiduMap.setMyLocationEnabled(true);
        mLocClient = BaiduMapHelper.buildLocationClient(this, new MyLocationListenner());
        if (mCurrentLantitude == 0 || mCurrentLongitude == 0)
            mLocClient.start();
        else {
            setLocation(mCurrentLantitude, mCurrentLongitude, 0);
            BaiduMapHelper.reverseGeoCode(mCurrentLantitude, mCurrentLongitude, onReverseGeoCodeResult);
        }
    }

    private BaiduMapHelper.OnReverseGeoCodeResult onReverseGeoCodeResult = new BaiduMapHelper.OnReverseGeoCodeResult() {
        @Override
        public void onReverseGeoCodeResult(ReverseGeoCodeResult reverseGeoCodeResult) {
            mCodeResult = reverseGeoCodeResult;
            if (reverseInList) { //在列表在选择，不更新
                reverseInList = false;
                return;
            }
            ...
            nearList = reverseGeoCodeResult.getPoiList();
            adapter = new LocNearAddressAdapter(MapActivity.this, nearList);
            lvLocNear.setAdapter(adapter);
        }
